...
def test_basic_export(geoarrow_con):
    tab = geoarrow_con.sql("""SELECT s2_geogfromtext('POINT (0 1)') as geom;""").to_arrow_table()
...
def test_basic_import(geoarrow_con):
    ...
    point_wkb = b"\x01\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00$@\x00\x00\x00\x00\x00\x004@"
    ...
    tab = geoarrow_con.sql("""SELECT s2_astext(geometry) as wkt FROM geo_table;""").to_arrow_table()
...
def test_roundtrip_countries(geoarrow_con):
    ...
    geo_table = geoarrow_con.sql(f"""SELECT s2_geogfromtext(geog) as geog1 FROM '{countries_file}'""").to_arrow_table()
    geo_table2 = geoarrow_con.sql("""SELECT geog1 as geog2 FROM geo_table""").to_arrow_table()
...
def test_spherely_interop(geoarrow_con):
    ...
    geo_table = geoarrow_con.sql("""SELECT geog, s2_area(geog) as area FROM s2_data_countries()""").to_arrow_table()
    geogs = spherely.from_geoarrow(geo_table["geog"].chunk(0))
    np.testing.assert_array_almost_equal(spherely.area(geogs), geo_table["area"].to_numpy(), decimal=1)
